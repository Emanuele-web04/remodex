...
// Purpose: Short provenance label for sidebar rows created by a desktop automation.
...
// Depends on: SwiftUI, CodexThread, AppFont
...
struct SidebarThreadSourceLabel: View {
    ...
    var body: some View {
        if let label = thread.automationSourceLabel {
            Text(label)
                ...
                .accessibilityLabel("Started by \(label) automation")
        }
    }
}
