...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
    guard let data = code.data(using: .utf8) else {
        return .scanError(L10n.tr("QR code contains invalid text encoding."))
    }
    ...
    if let payload = try? decoder.decode(CodexPairingQRPayload.self, from: data) {
        guard payload.v == codexPairingQRVersion else {
            return .bridgeUpdateRequired(
                makeScannerBridgeUpdatePrompt(
                    message: L10n.tr("This QR code was generated by a different Remodex npm version. Update the package on your Mac to the latest release before scanning a new QR code.")
                )
            )
        }

        guard !payload.relay.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return .scanError(L10n.tr("QR code is missing the relay URL. Re-generate the code from the bridge."))
        }

        guard !payload.sessionId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return .scanError(L10n.tr("QR code is missing the session ID. Re-generate the code from the bridge."))
        }
        ...
        guard expiryDate.addingTimeInterval(codexSecureClockSkewToleranceSeconds) >= now else {
            return .scanError(L10n.tr("This pairing QR code has expired. Generate a new one from the Mac bridge."))
        }
    ...
    if looksLikeRemodexPairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: L10n.tr("This QR code looks like it came from an older Remodex bridge. Update the npm package on your Mac to the latest release before scanning a new QR code.")
            )
        )
    }

    return .scanError(L10n.tr("Not a valid secure pairing code. Make sure you're scanning a QR from the latest Remodex bridge."))
}
...
private func makeScannerBridgeUpdatePrompt(message: String) -> CodexBridgeUpdatePrompt {
    CodexBridgeUpdatePrompt(
        title: L10n.tr("Update Remodex on your Mac before scanning"),
        message: message,
